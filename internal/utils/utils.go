...
//   1234567890       bare positive channel ID   (we add -100 prefix)
...
func ResolveChannelPeer(ctx *ext.Context, identifier string) (tg.InputPeerClass, error) {
	...
	// ── Username path ─────────────────────────────────────────────
	// Only reached if the identifier is NOT a valid integer.
...
// Handles both full negative IDs (-1001234567890) and bare positive IDs (1234567890).
func resolveChannelByID(ctx *ext.Context, id int64) (tg.InputPeerClass, error) {
	// ── Normalise to full negative ID ─────────────────────────────
	//
	// Telegram channel IDs come in two common forms:
	//   Full form : -1001234567890  (what the Bot API uses)
	//   Bare form :  1234567890     (what MTProto stores internally)
	//
	// The rule: if the number is negative and starts with -100 when printed,
	// it is already in full form. If positive (or negative but not -100),
	// treat it as a bare channel ID and prepend -100.

	var bareID int64 // the positive channel ID used in MTProto calls

	idStr := strconv.FormatInt(id, 10)

	switch {
	case strings.HasPrefix(idStr, "-100"):
		// Already full form: -1001234567890 → bare = 1234567890
	...
	default:
		// Negative but NOT -100 prefix (e.g. a regular group -1234567890).
		// Try resolving as a regular peer first.
		peer := ctx.PeerStorage.GetInputPeerById(id)
		if !peer.Zero() {
			return peer, nil
		}
		return nil, fmt.Errorf("cannot resolve peer with ID %d: not a channel and not found in peer storage", id)
	}

	// ── Try peer storage first (fast path) ───────────────────────
	// Peer storage uses the full negative ID as key.
	fullNegativeID := mustParseInt64("-100" + strconv.FormatInt(bareID, 10))
	peerInfo := ctx.PeerStorage.GetPeerById(fullNegativeID)

	if peerInfo.Type == int(storage.TypeChannel) {
		// We have the access hash cached, use it directly.
	...
	// ── Fallback: resolve via username won't help here.
	// Try InputChannel with zero access hash (works if bot is a member).
	resolved, err := ctx.Raw.ChannelsGetChannels(ctx, []tg.InputChannelClass{
		&tg.InputChannel{ChannelID: bareID, AccessHash: 0},
	})
	if err != nil {
		return nil, fmt.Errorf(
			"could not resolve channel ID %d (bare: %d): bot may not be a member of that channel: %w",
			fullNegativeID, bareID, err,
		)
	}
...
func extractChannelPeer(resolved tg.ChatsChatsClass, bareID int64) (tg.InputPeerClass, error) {
	...
	return nil, fmt.Errorf("channel ID %d resolved but no matching entity found in response", bareID)
}

// mustParseInt64 parses a string to int64, returns 0 on error (internal helper).
